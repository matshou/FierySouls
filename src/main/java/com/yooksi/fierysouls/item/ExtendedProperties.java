...
public class ExtendedProperties extends NBTTagCompound
{
	private static HashMap<ExtendedProperties, Fingerprint> ItemPropertiesMap = new HashMap();
	 ...
	 */
	public static ExtendedProperties createExtendedPropertiesForItem(final ItemStack item)
	{
		ExtendedProperties createdProperties = null;
		if (item != null && item.hasTagCompound() && findExtendedPropertiesForItem(item) == null)
		{
			...
			createdProperties = new ExtendedProperties();
	...
	{		
		for (Iterator<Map.Entry<ExtendedProperties, Fingerprint>> i = ItemPropertiesMap.entrySet().iterator(); i.hasNext();)
		{
		    Map.Entry<ExtendedProperties, Fingerprint> entry = i.next();
	 ...
	 */
	public static ExtendedProperties findExtendedPropertiesForItem(final ItemStack item)
	{
		ExtendedProperties foundProperties = null;
		for (Map.Entry<ExtendedProperties, Fingerprint> entry : ItemPropertiesMap.entrySet())
		{
			if (entry.getValue().validate(item.getTagCompound().getString("fingerprint")))
